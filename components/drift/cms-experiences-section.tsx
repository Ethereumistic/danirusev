...
interface CMSExperiencesSectionProps {
    experiences: DriftExperience[];
    linkPrefix?: string;
    title?: string;
    subtitle?: string;
}
...
/**
 * Experiences section that accepts pre-fetched and transformed experiences
 * Used by server components that fetch from CMS
 */
export function CMSExperiencesSection({
    experiences,
    linkPrefix = "/experience",
    title = "Избери Своя Адреналин",
    subtitle = "От пасажерска яхия до пълен контрол - имаме нещо за всеки"
}: CMSExperiencesSectionProps) {
    return (
